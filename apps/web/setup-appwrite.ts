...
async function main() {
  try {
    ...
    await setupCollection(
      ...
      "Daily Workouts",
      [
        Permission.read(Role.any()), // Anyone can read the daily workout
        Permission.create(Role.users()),
        Permission.update(Role.users()),
        Permission.delete(Role.users()),
      ],
      async (db, dbId, collId) => {
        await createStringAttribute(dbId, collId, "date", 255, true); // Unique PK essentially
        await createStringAttribute(dbId, collId, "workoutData", 100000, true); // JSON blob
        try {
